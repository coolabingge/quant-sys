...
class FirstFinishStrategy(object):
    """
    BBM策略执行器
    """
    ...
    def execute(self):
        # 开多开空判断
        macd = self.rst['MACD']
        macd_1h_much = float(self.rst_1h['MACD_DIFF']) > float(self.rst_1h['MACD_DEA'])
        macd_1h_less = float(self.rst_1h['MACD_DIFF']) < float(self.rst_1h['MACD_DEA'])
        current_line = str(self.rst['current_line'])

        self.stop_win_check()
        if not self.check_period_finish(current_line):
            return

        logger.info("非实时状态下进入策略匹配，当前Line线: %s, 当前价格：%s, 倍数： %s, macd: %s" % (
                current_line, self.cur_price, self.leverage, macd))
    ...
    def stop_win_check(self, strategy_index=None):
        ...
        for position in positions:
            ...
            if lose_rate_ok:
                logger.info("策略：4，环境：%s,用户：%s，当前仓位赢率：%s，达到损率条件, 平仓" % (self.env.name, self.user_id, win_rate))
            ...
            if win_rate != 0:
                logger.info("策略：4，，环境：%s, 用户：%s，仓位盈亏率: %s, 当前被动赢亏止损价：%s, 方向: %s" % (self.env.name, self.user_id, win_rate, lose_price, position.direction))
            ...
            if lose_price and (float(self.cur_price) <= lose_price) and (position.direction == 'long'):
                logger.info("策略：4，环境：%s, 用户：%s，方向：多, 被动止损触发金额: %s, 量：%s, 平仓" % (self.env, self.user_id, lose_price, stock_qty))
                self.empty_stock(stock_qty, win_rate, position.direction)
                self.market_trade.set_lose_price(user_id=self.user_id,
                                                 symbol_info=self.symbol_info,
                                                 price=None,
                ...
                logger.info("策略：4，环境：%s, 用户：%s，方向：空, 被动止损触发金额: %s, 量：%s, 平仓" % (self.env, self.user_id, lose_price, stock_qty))
                self.empty_stock(stock_qty, win_rate, position.direction)
                self.market_trade.set_lose_price(user_id=self.user_id,
                                                 symbol_info=self.symbol_info,
                                                 price=None,
