...
class FirstStrategy(object):
    ...
    def stop_win_check(self, strategy_index=None):
        ...
        for position in positions:
            ...
            if lose_rate_ok:
                logger.info("策略：1，环境：%s,用户：%s，当前仓位赢率：%s，达到损率条件, 平仓" % (self.env.name, self.user_id, win_rate))
            ...
            logger.info("策略：1，，环境：%s, 用户：%s，仓位盈亏率: %s, 当前被动赢亏止损价：%s, 方向: %s" % (self.env.name, self.user_id, win_rate, lose_price, position.direction))
            # 满足止赢率
            if win_rate_ok:
            ...
            if lose_price and (float(self.cur_price) <= lose_price) and (position.direction == 'long'):
                logger.info("策略：1，环境：%s, 用户：%s，方向：多, 被动止损触发金额: %s, 量：%s, 平仓" % (self.env, self.user_id, lose_price, stock_qty))
                self.empty_stock(stock_qty, win_rate, position.direction)
                self.market_trade.set_lose_price(user_id=self.user_id,
                                                 symbol_info=self.symbol_info,
                                                 price=None,
                ...
                logger.info("策略：1，环境：%s, 用户：%s，方向：空, 被动止损触发金额: %s, 量：%s, 平仓" % (self.env, self.user_id, lose_price, stock_qty))
                self.empty_stock(stock_qty, win_rate, position.direction)
                self.market_trade.set_lose_price(user_id=self.user_id,
                                                 symbol_info=self.symbol_info,
                                                 price=None,
